...
- 适合写入机器可读采纳清单的 Lithos 一致性声明：版本、单一全生命周期治理模型、角色、门禁、验证与自主 PR 策略，保持厂商中立、跨工具可移植。
...
本地工作流模板 [`templates/governed-ai-flow.md`](../../templates/governed-ai-flow.md) 仍可作为组件参考；本示例展示的是 Lithos 期望的完整生命周期结构，即使真实项目中的某些内容保持简洁也不应缺失这些结构锚点。
