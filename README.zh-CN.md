...
- **安全治理严格、状态治理轻量**——安全边界必须强验证；状态证据保持精简：路线图、当前状态与开发日志只记录阶段权威、当前决策、尾项和安全边界，不重复记录 git 历史、CI、PR 元数据或生成产物已经证明的例行账。见 [`docs/verification-standards.md`](docs/verification-standards.md)。
- **智能体运行清单**——逐次运行的审计记录：被授权做什么、实际运行了什么，以及涉及的证据与边界；它是记录，而非授权。见 [`docs/agent-run-manifest.md`](docs/agent-run-manifest.md)。
- **模板**——[`templates/`](templates/) 中可直接复制的起点：单独的本地工作流文件与完整项目起始骨架，二者都是同一个全生命周期治理模型的组件。
...
- **示例**——[`examples/`](examples/) 中的全生命周期模型落地范例。
...
3. 选择起点——二者都是同一个全生命周期模型的组件：[`templates/governed-ai-flow.md`](templates/governed-ai-flow.md) 是可单独复制的本地工作流文件，适合先保持简洁；完整 [`templates/governed-project/`](templates/governed-project/) 文档骨架则端到端展开同一模型，包含开发日志、经验、实践、生成索引、漂移报告与双语 README 规则。
