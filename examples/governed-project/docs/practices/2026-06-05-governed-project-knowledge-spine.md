...
6. Keep roadmap, current-status, and dev-log records lean: record phase authority, current decisions, open tails, and safety boundaries, but do not use status records as behavior/safety proof or duplicate generated evidence; split bulky history, evidence, or boundary detail into the right companion owner when the current-status entrypoint grows too large.
