...
- **Safety-strict, status-lean governance** — safety boundaries stay strongly verified while status evidence stays lean: roadmap, current-status, and dev-log records capture phase authority, current decisions, tails, and safety boundaries, not bookkeeping already proven by git history, CI, PR metadata, or generated artifacts. See [`docs/verification-standards.md`](docs/verification-standards.md).
