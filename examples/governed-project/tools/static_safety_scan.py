#!/usr/bin/env python3
"""Static safety scan for this governed project.

A first-class, machine-runnable governance gate. This pure-standard-library
scanner reads the project's committed text and rejects three classes of value
that must never enter a governed repository:

* secret-shaped tokens (credential, key, and access-token material);
...
2. **Self-test.** Before scanning, the engine runs on dynamically constructed
   probes -- secret-like strings built at runtime by concatenation, never
   stored as literals -- and confirms it flags the bad ones and clears a clean
   one. A reviewer can therefore trust that a clean scan means the engine works,
   not that it silently matched nothing.
"""
